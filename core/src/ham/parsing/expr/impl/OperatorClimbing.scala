...
abstract class OperatorClimbing[E](ops: List[Operator], whitespace: WhiteSpaceHandler) {
  ...
  private def binOp[_: P]: P[Operator] =
    binOpStr
      .filter(s => ops.exists(_.sym == s))
      .map(s => ops.find(_.sym == s).get)

  def opApp[_: P]: P[E] = P(expr1 ~ (binOp ~/ expr1).rep).map {
    case (pre, fs) =>
      var remaining = fs
